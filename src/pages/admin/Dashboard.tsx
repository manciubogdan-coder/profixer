...
const Dashboard = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data
      } = await supabase.from('subscription_statistics').select('*').single();
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {professionals.map(professional => <TableRow key={professional.id}>
                  <TableCell className="aici in tabelul acesta as vrea sa apara o singura data contul nu inteleg dece apare de atatea ori\n">
                    {professional.first_name} {professional.last_name}
                  </TableCell>
                  <TableCell>{professional.email}</TableCell>
                  <TableCell>
                    <span className={`px-2 py-1 rounded-full text-xs ${professional.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                      {professional.status === 'active' ? 'Activ' : 'Inactiv'}
                    </span>
                  </TableCell>
                  <TableCell>
                    {professional.end_date ? format(new Date(professional.end_date), 'dd/MM/yyyy') : 'Nesetat'}
                  </TableCell>
                  <TableCell>
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className="w-[240px]">
                          ...
                          {selectedEndDate && professional.id === selectedProfessionalId ? format(selectedEndDate, 'dd/MM/yyyy') : 'Selectează data expirării'}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0 !bg-black border-white/20" align="start">
                        <Calendar mode="single" selected={selectedEndDate} onSelect={date => {
                      setSelectedEndDate(date);
                      setSelectedProfessionalId(professional.id);
                    }} initialFocus />
                        <div className="p-3 border-t border-white/20">
                          <Button onClick={() => updateSubscriptionEndDate(professional.id)} disabled={!selectedEndDate || selectedProfessionalId !== professional.id} className="w-full">
